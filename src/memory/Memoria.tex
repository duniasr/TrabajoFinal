...
\begin{titlepage}
    ...
    {\Huge \textbf{TRABAJO FINAL}}\\[0.5cm]
    
    \includegraphics[width=\textwidth]{assets/caratula-trabajo.png} 
...
\subsection{Contexto y descripción del proyecto}
La interacción persona-ordenador (HCI) ha evolucionado drásticamente en las últimas décadas, transitando desde las abstractas líneas de comandos hasta las interfaces gráficas táctiles que dominan la actualidad. Sin embargo, la frontera actual de la tecnología busca ir un paso más allá: la eliminación total de la barrera física, dando lugar a las \textbf{interfaces naturales de usuario (NUI)}. En este contexto, la visión por computador se erige como una herramienta fundamental, permitiendo a las máquinas ``ver'' e interpretar el comportamiento humano de manera análoga a como lo hacemos las personas.

El presente proyecto, titulado \textbf{<<Simón Dice - Memorizador>>}, propone una reimaginación moderna e interactiva del clásico juego de memoria auditiva y visual de los años 80. La propuesta elimina los tradicionales botones de colores y mandos físicos, sustituyéndolos por un sistema de control basado enteramente en el reconocimiento de gestos faciales y manuales en tiempo real.
...
\begin{itemize}
    \item \textbf{Exploración de interfaces naturales (NUI)}: Existe un interés creciente en desarrollar sistemas que entiendan el lenguaje corporal humano. La motivación principal es demostrar que es posible crear interfaces fluidas y reactivas sin necesidad de hardware costoso o sensores de profundidad (como Kinect o LiDAR), utilizando únicamente algoritmos eficientes de inteligencia artificial sobre imágenes RGB convencionales.
    \item \textbf{Accesibilidad y usabilidad}: Los periféricos tradicionales (teclado, ratón, gamepads) pueden suponer una barrera para personas con movilidad reducida en los dedos o dificultades motrices finas. Un sistema basado en gestos amplios y reconocimiento facial abre la puerta a nuevas formas de ocio digital más inclusivas y accesibles.
    \item \textbf{Desafío del tiempo real}: El proyecto plantea el reto técnico de procesar vídeo de alta resolución, realizar inferencias con modelos de Machine Learning y gestionar la lógica de un videojuego multimedia, todo ello en intervalos de tiempo inferiores a 33 milisegundos (para mantener 30 FPS). Esta exigencia de optimización es una motivación constante para refinar el código y elegir las arquitecturas de red más eficientes.
...
\begin{enumerate}
    \item \textbf{Desarrollo de un pipeline de visión robusto}: Implementar un flujo de procesamiento de imágenes que integre la captura, preprocesamiento y extracción de puntos clave (\textit{landmarks}) mediante la librería MediaPipe, garantizando la detección estable de manos y rostro bajo diferentes condiciones de iluminación.
    \item \textbf{Creación y entrenamiento de modelos de clasificación}: Generar un \textit{dataset} propio, diverso y equilibrado de gestos faciales y manuales. Diseñar, entrenar y validar perfiles de redes neuronales (Perceptrón Multicapa - MLP) capaces de clasificar estos gestos con una precisión superior al 95\%.
    \item \textbf{Implementación de lógica de juego y máquina de estados}: Programar una arquitectura de software modular que gestione los estados del juego (menú, turno del sistema, turno del jugador, evaluación), asegurando la sincronización entre las instrucciones audiovisuales y la respuesta del usuario.
    \item \textbf{Integración multimedia y feedback de usuario}: Desarrollar un sistema de retroalimentación claro que informe al jugador de sus aciertos y errores mediante efectos sonoros y visuales en pantalla, minimizando la latencia percibida.
    \item \textbf{Optimización del rendimiento}: Asegurar que el coste computacional del sistema permita su ejecución fluida en CPUs convencionales, evitando el uso obligatorio de GPUs dedicadas para garantizar la portabilidad del software.
\end{enumerate}
...
\subsubsection{Pipeline de procesamiento de datos}
El flujo de transformación de los datos sigue un proceso secuencial diseñado para maximizar la eficiencia. En la Figura \ref{fig:pipeline_tikz} se detalla este esquema.

\begin{figure}[H]
    \centering
    \shorthandoff{>} % DESACTIVA el conflicto con Babel español
    \begin{tikzpicture}[
        node distance=1cm and 1.5cm,
        block/.style={rectangle, draw, fill=blue!10, text width=2.8cm, align=center, rounded corners, minimum height=3em},
        arrow/.style={thick,->,>=stealth}
    ]
        % Nodos
        \node (cam) [block] {Entrada: Cámara (Frame RGB)};
        \node (mp) [block, right=of cam] {Inferencia MediaPipe (Landmarks)};
        \node (norm) [block, right=of mp] {Normalización y Centrado};
        \node (mlp) [block, below=of norm] {Clasificador MLP (.pkl)};
        \node (game) [block, left=of mlp, xshift=-1cm] {Lógica del Juego (Comando)};
    ...
    \end{tikzpicture}
    \shorthandon{>} % REACTIVA Babel español
    \caption{Esquema del flujo de datos del sistema.}
    \label{fig:pipeline_tikz}
\end{figure}

Como se observa, el proceso consta de las siguientes etapas:
\begin{itemize}
    \item \textbf{Captura e Inferencia}: MediaPipe localiza los \textit{landmarks} espaciales.
    \item \textbf{Preprocesamiento}: Se extraen las coordenadas $(x, y)$ y se aplican técnicas de centrado y escalado unitario.
    \item \textbf{Vectorización}: La matriz de puntos se aplana en un vector unidimensional apto para la entrada de la red neuronal.
    \item \textbf{Inferencia MLP}: El Perceptrón Multicapa asigna una probabilidad a cada clase de gesto.
\end{itemize}
...
\subsubsection{Clasificación con perceptrón multicapa (MLP)}

El núcleo de decisión del sistema se basa en un \textbf{perceptrón multicapa (MLP)}, una red neuronal artificial de tipo \textit{feedforward}. A diferencia de las redes convolucionales (CNN) que extraen características directamente de los píxeles de la imagen, nuestro enfoque se basa en la \textbf{ingeniería de características} (\textit{Feature Engineering}) previa realizada por MediaPipe.

Se ha optado por una \textbf{arquitectura unificada y ligera} para ambos modelos (facial y corporal), priorizando la velocidad de procesamiento en tiempo real:

\begin{itemize}
    \item \textbf{Arquitectura de la red (topología):}
    \begin{itemize}
        \item \textbf{Capa de entrada:} Variable según el modelo.
        \begin{itemize}
            \item \textit{Manos}: 42 neuronas ($21 \text{ puntos} \times 2 \text{ ejes}$).
            \item \textit{Cara}: 936 neuronas ($468 \text{ puntos} \times 2 \text{ ejes}$).
        \end{itemize}
        \item \textbf{Capas ocultas:} $(64, 32)$. Se utiliza una estructura de "embudo". 
        \item \textbf{Capa de salida:} Neuronas correspondientes al número de clases a predecir (9 para manos, 4 para cara).
    \end{itemize}
\end{itemize}

\paragraph{Justificación de la arquitectura (64, 32):}
A pesar de que la malla facial cuenta con 468 puntos (936 valores de entrada), se demostró que una primera capa oculta de solo \textbf{64 neuronas} es suficiente para extraer las características relevantes.
\paragraph{Hiperparámetros del entrenamiento:}
Para ambos modelos se utilizó la implementación \texttt{MLPClassifier} de la librería \textit{Scikit-Learn} con la siguiente configuración:
\begin{itemize}
    \item \textbf{Función de Activación}: \texttt{ReLU} (Rectified Linear Unit), elegida por su eficiencia computacional.
    \item \textbf{Solver (Optimizador)}: \texttt{Adam}, un algoritmo de optimización estocástica eficiente.
    \item \textbf{Iteraciones máximas}: \texttt{max\_iter=1500}. Se estableció un límite alto para asegurar la convergencia total del modelo, dado que el tamaño reducido de la red minimiza el riesgo de sobreajuste (\textit{overfitting}) y el tiempo de cómputo sigue siendo bajo.
    \item \textbf{Semilla aleatoria}: Se fijó \texttt{random\_state=42}. Esto anula la aleatoriedad en la inicialización de los pesos sinápticos y en la mezcla de datos (\textit{shuffling}), garantizando la \textbf{reproducibilidad} exacta de los resultados y métricas presentados en este documento.
\end{itemize}
...
\subsection{Extracción de características vs. redes convolucionales}
...
\subsection{Bloqueo por librerías de audio (pyttsx3)}
El uso inicial de la librería \texttt{pyttsx3} para la síntesis de voz en tiempo real provocaba bloqueos en el hilo principal de ejecución, congelando el vídeo durante la narración de instrucciones. \\
\textbf{Solución}: Se sustituyó la síntesis en tiempo real por el uso de clips de audio pre-grabados generados mediante una Inteligencia Artificial de voz neuronal. Se implementó una función asíncrona basada en \texttt{pygame.mixer} y una gestión de cola de reproducción para integrar estos audios sin afectar a los FPS del juego.
...
\begin{enumerate}
    ...
    \item \textbf{Órdenes válidas}: Si dice ``Simón dice: [Acción]'', debes imitar el gesto.
    \item \textbf{Trampas}: 
    \begin{itemize}
...
\section{Propuestas de ampliación y trabajo futuro}
...
\subsection{Reconocimiento de gestos dinámicos}
A pesar de la solidez del sistema actual, existe una limitación inherente a su diseño: el modelo clasifica \textbf{posturas estáticas}. El sistema entiende si una mano está abierta o cerrada en un instante concreto ($t$), pero carece de contexto temporal para entender el movimiento que llevó a esa postura.

Para llevar el proyecto al siguiente nivel y permitir una interacción más rica, se propone la siguiente evolución técnica:

\subsubsection{De la foto al vídeo: redes recurrentes}
La ampliación natural consiste en dotar al sistema de ``memoria''. Técnicamente, esto implicaría sustituir o complementar el clasificador actual por una \textbf{red neuronal recurrente (RNN)}, específicamente una arquitectura \textbf{LSTM (Long Short-Term Memory)}.

En lugar de alimentar al modelo con un único \textit{frame}, se utilizaría una ventana deslizante de los últimos $N$ fotogramas (por ejemplo, los últimos 0.5 segundos). Esto permitiría al sistema analizar no solo la posición de los puntos, sino su \textbf{trayectoria y velocidad}.

\subsubsection{Nuevas mecánicas de juego}
Esta capacidad de análisis temporal abriría la puerta a gestos complejos que actualmente son imposibles de detectar:
\begin{itemize}
    \item \textbf{Gestos de trayectoria:} Instrucciones como ``Saluda con la mano'' o ``Dibuja un círculo en el aire''.
    \item \textbf{Asentimiento y negación:} Detectar si el usuario mueve la cabeza diciendo ``Sí'' o ``No'', en lugar de solo detectar si mira a la derecha o izquierda.
    \item \textbf{Análisis de velocidad:} Mecánicas donde el juego pida realizar una acción ``a cámara lenta'' o ``lo más rápido posible''.
\end{itemize}

Esta evolución requeriría la generación de un nuevo dataset basado en secuencias de vídeo, aumentando la complejidad del entrenamiento, pero elevando exponencialmente la naturalidad de la interacción hombre-máquina.

\subsection{Adaptabilidad emocional y cognitiva}
...
\begin{itemize}
    \item \textbf{Ajuste de dificultad dinámico}: Si el sistema detecta micro-expresiones de frustración o estrés en el jugador, podría reducir la velocidad de la secuencia de ``Simón'' automáticamente. Por el contrario, si detecta aburrimiento, podría acelerar el ritmo.
    \item \textbf{Feedback empático}: El juego podría reaccionar a las expresiones del usuario, por ejemplo, animándolo si falla o celebrando con él si sonríe tras un acierto.
\end{itemize}

\subsection{Soporte multiplataforma y móvil}
Actualmente, el juego se ejecuta en PC. Una línea de trabajo futura es la migración a arquitecturas móviles (Android/iOS).
\begin{itemize}
    \item \textbf{Optimización TFLite}: Convertir los modelos entrenados al formato TensorFlow Lite para permitir la inferencia en dispositivos con recursos limitados (Edge Computing).
    \item \textbf{Sensores inerciales}: En un entorno móvil, se podrían fusionar los datos de la cámara frontal con los del giroscopio y acelerómetro del teléfono para detectar gestos de inclinación de la cabeza con mayor precisión.
\end{itemize}

\subsection{Multijugador online (WebRTC)}
Romper la barrera local y permitir que dos jugadores compitan desde ubicaciones remotas.
\begin{itemize}
    \item \textbf{Arquitectura cliente-servidor}: Implementar un servidor de señalización (usando WebSockets) y protocolos WebRTC para transmitir el vídeo y los datos de la partida en tiempo real con baja latencia.
    \item \textbf{Competitividad}: Crear un ranking global de puntuaciones almacenado en la nube.
\end{itemize}
