...
El núcleo de decisión del sistema se basa en un **Perceptrón Multicapa (MLP)**, una red neuronal artificial de tipo \textit{feedforward}. A diferencia de las redes convolucionales (CNN) que extraen características directamente de los píxeles de la imagen, nuestro enfoque se basa en la \textbf{Ingeniería de Características} (\textit{Feature Engineering}) previa realizada por MediaPipe.
...
A pesar de que la malla facial cuenta con 468 puntos (936 valores de entrada), se demostró que una primera capa oculta de solo **64 neuronas** es suficiente para extraer las características relevantes.
...
\begin{itemize}
    ...
    \item \textbf{Semilla aleatoria}: Se fijó \texttt{random\_state=42}. Esto anula la aleatoriedad en la inicialización de los pesos sinápticos y en la mezcla de datos (\textit{shuffling}), garantizando la **reproducibilidad** exacta de los resultados y métricas presentados en este documento.
\end{itemize}
